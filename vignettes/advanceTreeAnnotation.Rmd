...
For associating phylogenetic tree with different type of plot produced by user's data, `ggtree` provides `alnplot` function which accepts an input data frame and a geom function to draw the input data. The plot generated by the geom function will automatically align to the tree. 
...
p2 <- alnplot(p, panel="dot", data=d1, geom=geom_point, aes(x=val), color='firebrick')
d2 <- data.frame(id=tr$tip.label, value = abs(rnorm(30, mean=100, sd=50)))

alnplot(p2, panel='bar', data=d2, geom=geom_segment, aes(x=0, xend=value, y=y, yend=y), size=3, color='steelblue') + theme_tree2()
```
