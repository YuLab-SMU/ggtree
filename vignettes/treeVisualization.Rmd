---
title: "Simple Tree Visualization"
author: "\\
...
output: 
  ...
  BiocStyle::pdf_document:
    toc: true
vignette: >
  %\VignetteIndexEntry{02 Simple Tree Visualization}
...
To view a phylogenetic tree, we first need to parse the tree file into `R`. The `r Githubpkg("GuangchuangYu/ggtree")` package supports many file format including output files of commonly used software packages in evolutionary biology. For more details, plase refer to the [Tree Data Import](treeImport.html) vignette.
...
The `r Githubpkg("GuangchuangYu/ggtree")` package extends _`ggplot2`_ package to support viewing phylogenetic tree.
...
In this example, the figure demonstrates the quantity of __*y*__ increase along the trunk. User can highlight the trunk with different line size or color using the functions described in __XXX__.
...
Users can use _`geom_text`_ to display the node (if availabel) and tip labels simultaneously or `geom_tiplab` to only display tip labels:
